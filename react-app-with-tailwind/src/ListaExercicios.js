...
const ListaExercicios = () => {
  ...
  const salvar=()=>{
    if(aluno.id!==0){
      axios.put(`http://localhost:3004/exercises/${exercicio.id}`,aluno).catch((erro)=>console.log(erro))
    }else{
      axios.post('http://localhost:3004/exercises',exercicio).catch((erro)=>console.log(erro))
    }
  }
  ...
  const editar=(e)=>{
    const id = parseInt(e.target.id);
    axios.get(`http://localhost:3004/exercises/${id}`)
    .then((exercicio)=>{
      setAluno(exercicio.data);
      setShow(true);
    })
    .catch((erro)=>console.log(erro))
  }
